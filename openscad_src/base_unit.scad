...
module mock_motor(hole_shadow_depth = 7, gear_shadow = true, square_gear = true, nema23 = false) {
    hole_shadow_width = 3.5;
    hole_shadow_length = 10;
    cw = nema23 ? Nema23_cube_width : Nema17_cube_width;
    ridge = nema23 ? Nema23_ring_diameter / 2 : Nema17_ridge_r;
    gl = gear_length + 5;
    color("steelblue")
        ...
        rotate([0, 90, 0])
          if (square_gear) {
            ...
            translate([-cw / 2, cw / 2, 0.1])
              cylinder(r = ridge, h=gl, $fn=64);
          }
        //gear shadow
        if (gear_shadow) {
            translate([gl / 2 + 0.1, cw / 2, cw / 4])
                cube([gl, ridge * 2, cw / 2], center=true);
        }
        //mounting holes
        rotate([0, 90, 0])
        translate([-cw / 2, cw / 2, gl - motor_mount_depth - hole_shadow_depth + 0.1])
          union() {
            screw_holes(3.5, motor_mount_depth + hole_shadow_depth, $fs=1, nema23=nema23);
            //mounting hole shadow
            screw_translate(4, nema23) {
...
//projection(cut = true)
//translate([-base_w / 2, -base_d / 2, -4 * layer_height + 0.1])
base_unit();
module base_unit() {
    union() {
        difference() {
            ...
            rotate([0, 90, 0])
                mock_motor(hole_shadow_depth=100, square_gear=false, nema23=true);
            D_anchor(0);
            D_anchor(1);
            D_anchor(2);
            registration_holes();
